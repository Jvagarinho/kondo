...
const FormTextArea = ({
  ...
  const getCharCountColor = () => {
    if (maxLength && charCount > maxLength * 0.9) {
      return charCount === maxLength ? '#ef4444' : '#f59e0b';
    }
    ...
    <div className={`form-field ${className}`}>
      ...
      <div className="form-input-wrapper">
        <textarea
          ...
          rows={rows}
          maxLength={maxLength}
          className={`form-input form-textarea ${getStatusClass()} ${showValidationIcon ? 'with-validation' : ''}`}
          style={{
            borderColor: getStatusColor(),
            paddingRight: showValidationIcon ? '2.75rem' : '1rem'
          }}
        />
